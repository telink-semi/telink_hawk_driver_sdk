...
void main_loop (void)
{
	...
	while(1)
	{
		...
		timeout_state=0;
		delay_ms(100);
		rf_start_stx2rx (Zigbee_tx_packet, get_sys_tick()+16*TX_DELAY_US,STX_WAITTIME_US);
		while(1)
		{
			if(tx_state==1)//tx
			{
	...
	while(1)
	{
		...
		rx_state=0;
		timeout_state=0;
		rf_start_srx2tx (Zigbee_tx_packet, get_sys_tick()+16*TX_DELAY_US,SRX_WAITTIME_US);
		while(1)
		{
			if(tx_state==1)//tx
			{
